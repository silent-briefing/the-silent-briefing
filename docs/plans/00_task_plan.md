...
**Refined Master Plan Complete.** Phase 1 backend: **Steps 0–1 complete** (worker CLI, baseline ETL, judicial/retention/opinion paths). **Step 2** in progress (graph + claims + vectors + `match_rag_chunks` landed; enum/RLS hardening still open per Step 2 status). **Step 3** (autonomous researcher / dossier pipeline) pending. **Current focus:** `01_expanded` **Phase 2** (Directus CMS / judicial dossier UX) and **Step 2** enum/RLS hardening as needed. Phase **1.9** schedule catalog + `correlation-recent-chunks` shipped (`worker --dry-run`). Re-read `plans/02_findings.md`, `plans/03_progress.md`, and `plans/04_foundation_supabase_directus.md` before major DB/CMS changes.
...
- **FastAPI** app: health, version, **REST** routes for triggers/status (e.g. “run scrape”, “enqueue candidate X”, “job status”) — thin controllers, fat **services** modules.
- `**pydantic-settings`:** env-based config — `**SUPABASE_URL`** + `**SUPABASE_SERVICE_ROLE_KEY`** (or split anon vs service keys) from `**supabase start`** locally, dashboard when hosted; optional `**DATABASE_URL`** for direct SQL/SQLAlchemy (`postgresql://postgres:postgres@127.0.0.1:54322/postgres` local default per CLI output); Perplexity, Cloudflare, etc.; **never** embed `service_role` in client bundles.
- **Lifespan:** open shared **HTTP clients** / connection pools in [FastAPI lifespan](https://fastapi.tiangolo.com/advanced/events/); teardown on shutdown.
- **Long work:** `**[BackgroundTasks](https://fastapi.tiangolo.com/tutorial/background-tasks/)`** only for **light** post-response work; **scrape + Sonar + writer + embed** run in a **separate worker process** or **queue-backed job** (FastAPI docs point to **Celery**-class tools for heavy work — pick ARQ/RQ/Huey/Celery or a dedicated `worker` CLI; document choice).
- **Scheduling:** **GitHub Actions cron**, **K8s CronJob**, or host **cron** invoking a **worker module** (e.g. `python -m <package>.worker run-nightly`) or HTTP to a **secured** admin route — FastAPI has **no** built-in cron.
- **Optional later:** **WebSockets** for job progress — [FastAPI websockets](https://fastapi.tiangolo.com/advanced/websockets/) (single-process story; scale-out needs pub/sub — out of scope until required).
- **Queue / worker default (document in README):** **ARQ + Redis** for async I/O-heavy pipelines (scrape, LLM, embed); **Celery** if the team standardizes on it or needs broader workflow primitives; **plain asyncio worker + cron** only with **DB-backed claiming** (`FOR UPDATE SKIP LOCKED`, leases) and documented risk of overlapping cron runs. Optional: **Supabase Queues** ([pgmq](https://supabase.com/docs/guides/queues)) instead of Redis — different ops tradeoff, same need for **idempotent** side effects.
- **Idempotency:** unique `**idempotency_key`** (or natural keys) on job rows / upserts so retries and duplicate deliveries do not double-charge APIs or duplicate claims.
- **Status:** complete (skeleton in `backend/` — `uv sync`, `uvicorn briefing.api.main:app`, `**GET /health`**, `**GET /version`**, FastAPI lifespan with shared `**httpx.AsyncClient**` on `app.state` + `**briefing.api.deps.get_http_client**`, `**X-Request-ID**` middleware for correlation; worker via `**python -m briefing.worker**` subcommands). **ARQ / Redis queue not wired yet** — plan text below still describes a future default; heavy work runs in the worker CLI or should move to a queue when added.
...
- **Tooling (per probe):**
  ...
  - **SLCO:** **Playwright** (Chromium): open list URL → first `<select>` **All** → wait → scrape **h3** (office) + following **div** (candidate blobs; split on `Party - Filed` markers via regex). Toggle `SLCO_PLAYWRIGHT_ENABLED=false` to skip. **Cloudflare `/content`** remains an ops alternative.
  - **Other:** Firecrawl optional per host (not wired).
- **Supplement:** [Google Civic Information API v2](https://developers.google.com/civic-information/docs/v2) — `**voterInfoQuery`** → `NormalizedCandidate` when `**GOOGLE_CIVIC_API_KEY`** and `**GOOGLE_CIVIC_VOTER_ADDRESS**` set (optional `**GOOGLE_CIVIC_ELECTION_ID**`). Standalone `**electionsQuery**` (`fetch_google_civic_elections`) and `**divisionsByAddress**` (`fetch_google_civic_divisions_by_address`) in `briefing.services.baseline.civic` — artifacts optional; **no** Representatives endpoint wrapper.
- **Output contract (normalized list):** `NormalizedCandidate` — Name, Office Sought, Party, Incumbency (empty when unknown), District (parsed from office text), provenance fields.
- **Operations (smoke / schedule):** `[.github/workflows/baseline-extraction-smoke.yml](.github/workflows/baseline-extraction-smoke.yml)` — `workflow_dispatch` + daily UTC cron; runs `uv sync`, Playwright system deps + Chromium, `python -m briefing.worker baseline-extraction` (smoke uses `--dry-run --sources vote_utah`). **Idempotent merge** into `**public.races` / `public.candidates`** via upsert (`dedupe_key` + race natural key). Production may call **secured API** (`POST /v1/extraction/baseline-sync`) or the **same worker CLI** on a schedule.
- **Resilience (MVP):** `briefing.services.baseline.fetcher` — `resilient_get_text` and `**ResilientFetcher`**: retries, backoff, Retry-After on 429, optional `**If-None-Match`** + on-disk body cache under `EXTRACTION_ARTIFACTS_DIR/.fetch_catalog/` (body SHA-256 in meta), and a **bounded semaphore** for concurrent GETs. `vote_utah` passes `extraction_artifacts_dir` when set.
- **Artifacts:** raw HTML + retention (`EXTRACTION_ARTIFACTS_DIR`, `EXTRACTION_ARTIFACTS_RETENTION_DAYS`) — vote.utah response, SLCO httpx probe, SLCO Playwright render. **Discovery vs fetch:** when `EXTRACTION_ARTIFACTS_DIR` is set, `ResilientFetcher` stores **body SHA-256** per URL; an unchanged hash + **304** path means listing HTML is unchanged (cheap skip signal for downstream parses). Full separate job types remain an ops refinement if you need queue-friendly manifests.
- **Status:** complete for roster pull + **Supabase upsert** when service role configured (`persist_baseline_extraction` in `briefing.services.persistence`, `POST /v1/extraction/baseline-sync`, CLI `**baseline-extraction --persist`**).
...
- `**races`** — table + unique `(office_label, district, jurisdiction)` in `[supabase/migrations/20260319180000_races_candidates.sql](supabase/migrations/20260319180000_races_candidates.sql)` (RLS on; **service_role** grants only).
- `**candidates`** — FK to `races`, `dedupe_key` column + unique index; `**briefing.services.persistence.baseline_upsert`** upserts from `NormalizedCandidate` rows (vote.utah + SLCO + Civic voterInfo).
- `**entities`** — unified node table (minimal enum in `[20260319180000_races_candidates.sql](supabase/migrations/20260319180000_races_candidates.sql)`); extend types / dedupe strategy still TBD (see Key Questions).
- `**entity_edges`** — landed in `[20260420120000_entities_claims_vectors_graph.sql](supabase/migrations/20260420120000_entities_claims_vectors_graph.sql)`: `**source_entity_id**`, `**target_entity_id**`, `**relation**`, `**provenance**`, `**status**` (`proposed` | `accepted` | `rejected`), `**confidence**` / `**weight**` / `**valid_from**`.
- `**dossier_claims**` — same migration + `[20260420120100_link_officials_claims.sql](supabase/migrations/20260420120100_link_officials_claims.sql)`: FKs `**candidate_id**`, `**official_id**`, subject/object `**entities**`; `**pipeline_stage**` enum (`retrieval_sonar` | `writer_sonar` | `critique_sonar` | `human_edit`); LLM provenance columns; `**groundedness_score**`.
- `**rag_chunks**` — same migration: `**content**`, `**embedding vector(1024)**` (align dim with embed API in Phase 1.5), `**embedding_model_id**`, `**source_url**`, `**source_type**`, `**chunk_index**`, `**content_hash**`.
- `**intelligence_runs**` — same migrations: `**model_id**`, `**pipeline_stage**`, `**status**`, tokens/cost, `**raw_response**`, `**idempotency_key**`, `**groundedness_score**`, `**requires_human_review**`; FKs `**candidate_id**`, `**official_id**`.
...
- **ANN + `match_rag_chunks`:** HNSW cosine index + `public.match_rag_chunks(vector(1024), int)` in `[20260420140000_rag_chunks_ann_match.sql](supabase/migrations/20260420140000_rag_chunks_ann_match.sql)` (`service_role` execute only). Feeds Phase 1.5 RAG path in `plans/01_expanded_silent_briefing_platform_plan.md`.
- **Status:** in progress (graph + claims + vectors + runs + **rag ANN/RPC** landed 2026-04-20; entity enum expansion / full RLS review still open)
...
- Core layout and scheduling patterns established in original plan.
...
**Objective:** Supreme Court focus. Extract opinions/bios from utcourts.gov + Ballotpedia (retention vote history → `dossier_claims` via **Phase 1.4** in `plans/01_expanded_silent_briefing_platform_plan.md`). Build CorrelationEngine for automatic edge proposal using cheap Perplexity calls. Enable "everything referenced" + intelligent reporting.
