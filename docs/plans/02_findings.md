...
**Dev workflow (2026-04-21):** Prefer `**supabase migration up`** via `**scripts/dev-db-migrate.***` for local schema changes so **Directus metadata and users stay** in Postgres; use full `**dev-db-reset.*`** only when intentionally replaying all migrations from a clean DB.
