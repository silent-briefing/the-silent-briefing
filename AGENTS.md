...
- **Operator console** code lives in `console/` (Bun + Next.js App Router). Package name is `silent-briefing-console` because npm reserves the name `console`. Clerk middleware: public `/sign-in`, `/sign-up`, `/api/health`; `/admin/*` requires `public_metadata.role === "admin"`; missing role defaults to **viewer** for non-admin routes.