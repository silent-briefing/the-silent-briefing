...
- **Operator console (`console/`):** use **`clerkMiddleware`** in **`src/proxy.ts`** (Next.js 16 renamed the `middleware` file convention to **`proxy`** — same Edge behavior). Enable **Organizations** in the Clerk Dashboard for multi-tenant orgs.
- **App roles (three):** `admin`, `operator`, `viewer`. **Effective role** = the higher of (1) **`public_metadata.role`** on the Clerk user (normalized to those three strings) and (2) **Clerk organization admin** when an org is active on the session JWT (`org_role` of `org:admin` or legacy `admin`, or abbreviated claim `o.rol === "admin"`). Middleware treats a fully missing effective role as **`viewer`**; `/admin/*` requires **`admin`**. Org admins must **select that organization** in the Clerk org switcher so org claims are present. JWT template **`supabase`** should pass `role`, `org_id`, `sub`, and `aud: "authenticated"` per `docs/plans/04_foundation_supabase_directus.md`. FastAPI uses the same merge in **`backend/briefing/api/deps_auth.py`** (`effective_app_role_from_payload`).
...
- **Clerk:** **Organizations** enabled; **three app roles** — `admin`, `operator`, `viewer` — from merged **`public_metadata.role`** and **org admin** JWT claims (`org_role` / `o.rol`); `/admin/*` requires effective **admin**. Default for signed-in users with no matching claims is **viewer** on non-admin routes (see Authentication section above for middleware detail).
...
- **CI:** `.github/workflows/gui-ci.yml` — typecheck, lint, Vitest, production build, Playwright (axe on listed operator routes), **Lighthouse CI (blocking)** on URLs in **`console/lighthouserc.cjs`**, backend **`pytest`**.
- **Phase B (operator console — shipped):** Briefing home, Judicial Watch, shared dossier (`/judicial/[slug]`, `/officials/[slug]`), Officials hub + compare + search + graph, BFF feeds/search, saved views + alerts (**B.12**), primitives + data layer from plan B.2–B.3. **`NEXT_PUBLIC_SEARCH_SEMANTIC=0`** disables client semantic calls when the embed/BFF path is unavailable.
...
- **Phase B.13 — A11y + performance:** Root **`SkipToContent`** → **`#site-main`** on operator/admin/sign-in **`main`** landmarks (`tabIndex={-1}` for skip focus). Dossier **`DossierTabsShell`** moves focus into the active tab panel on change. Global **`prefers-reduced-motion`** short-circuits animations/transitions; roster pinstripe hover/focus inset is suppressed under reduced motion. Playwright axe: **`e2e/a11y-routes.spec.ts`** uses **`iframes: false`** and a **critical-only** violation gate (hosted Clerk / embedded account UI is out of scope for the automated axe pass; tighten with a signed-in **`storageState`** when you need stricter WCAG on operator HTML). Routes include **`/officials/justice-hagen`** plus hubs. **`lighthouserc.cjs`**: **accessibility** min score **0.93** (**error** — fails CI); **LCP** max **3200** ms (**warn** — reporting slack on desktop cold start, not a hard gate unless LHCI config changes).
