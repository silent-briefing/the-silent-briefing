...
- **Phase B.13 — A11y + performance:** Root **`SkipToContent`** → **`#site-main`** on operator/admin/sign-in **`main`** landmarks (`tabIndex={-1}` for skip focus). Dossier **`DossierTabsShell`** moves focus into the active tab panel on change. Global **`prefers-reduced-motion`** short-circuits animations/transitions; **`Card`** / **`Portrait`** use **`motion-reduce:transition-none`**. Playwright axe covers expanded operator routes; **`lighthouserc.cjs`** adds `/search`, `/graph`, sample dossier **`/judicial/justice-hagen`**, tighter accessibility assertion (**error** ≥ 0.92), LCP **warn** ≤ 4000ms; **`gui-ci.yml`** Lighthouse job is no longer **`continue-on-error`**.
