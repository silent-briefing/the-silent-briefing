...
- **Primary**: Perplexity for *everything* (one-stop). Use **Sonar** variants (including new Sonar Reasoning Pro for multi-step CoT, correlation, edge extraction—128k context, great for opinion analysis). Agent API for writing, structured output, tools. Configurable models.
- **Abstraction**: `services/llm/PerplexityClient` (or abstract LLMService) with config for provider, model map, fallback chain. Easy to swap (add Anthropic/OpenAI/xAI later).
- **Adversarial for Reports**: For final dossiers/briefs (especially judicial): Primary draft (Perplexity large/Agent), Adversarial critique pass (different pedigree/model e.g. Sonar Reasoning Pro with "find flaws, contradictions to sources, missing cross-refs, unsubstantiated claims" prompt, or configured Grok/Claude via fallback). Then synthesis pass. "Make them talk": Sequential or parallel calls with debate-style prompts. Research note: Adversarial agents can persuade toward errors (10-40% accuracy drop); mitigate with strict grounding to Stage 1 evidence, provenance requirements, citation validation, human final review queue (in Directus/Console). Use cheaper models for high-volume correlation/edge proposal.
